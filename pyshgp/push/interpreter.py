...
from pyshgp.push.instruction import Instruction, MemoryInstruction
...
class MemoryInterpreter(PushInterpreter):
    ...
    def __init__(self,
        ...
        self.dementia_amt = dementia_amt

    def _evaluate_instruction(self, instruction: MemoryInstruction, config: PushConfig):
        self.state = instruction.evaluate(self.state, self.memory, config)
    ...
    def evaluate_atom(self, atom: Atom, config: PushConfig):
        ...
        try:
            if isinstance(atom, InstructionMeta):
                if "mem_" in atom.name:
                    self._evaluate_instruction(self.instruction_set[atom.name], config)
                else:
                    # This one wont do anything with memory
                    super()._evaluate_instruction(self.instruction_set[atom.name], config)
            elif isinstance(atom, Input):
                input_value = self.state.inputs[atom.input_index]
                self.state.untyped.append(input_value)
            elif isinstance(atom, CodeBlock):
    ...
    def run(self,
        ...
        while len(self.state["exec"]) > 0:
            ...
            # Reset the memory
            if self.dementia_amt != 0 and steps % self.dementia_amt == 0:
                for num in range(len(self.memory)):
                    self.memory[num] = 0
